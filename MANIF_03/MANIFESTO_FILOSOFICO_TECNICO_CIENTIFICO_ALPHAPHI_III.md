...
### ECO-NN Operacional — Primeira Execução e o Limiar de Neutralização

### I. Enunciado do Pesquisador
> [Resultado experimental — ECO-NN com ECO-BIP no forward pass + regularização angular]
— Vitor Edson Delavi, 19 de setembro de 2026
...
Primeiro teste do ECO-NN operacional: ECO-BIP integrado no forward pass de cada camada (diferenciável via torch.fft) + regularização angular na loss. Dois estágios de treino:
- Estágio 1: 200 épocas MSE puro (estabilizar reconstrução)
- Estágio 2: 300 épocas MSE + L_angular (λ=0.15)

#### II.2 — Resultados

```
θ_global (crista)    : +42.30°   ← era −16.1° (AP MSE puro)
θ_local médio        : +90.16°   ← invariante em todas as configurações
θ_R referência       : +63.43°
Δ_global             : 21.14°    ← era 79.5°
% em θ_R ± 5°        : 0.0%

Betas SEAL aprendidos: β ≈ 0.18–0.23 por camada
SEAL limite          : 1/φ = 0.6180
```

#### II.3 — A Tabela de Progressão

| Método | θ_global | Δ de θ_R |
|---|---|---|
| Campo virgem (sem treino) | +41.6° | 21.8° |
| AP treinado — MSE puro | −16.1° | 79.5° |
| Lente B — ECO-BIP pós-processamento | +57.8° | 5.6° |
| **ECO-NN operacional (integrado)** | **+42.3°** | **21.1°** |

#### II.4 — O Limiar de Neutralização

A observação central: **+42.3° ≈ campo virgem (+41.6°)**. O ECO-NN operacional não superou o campo virgem — restaurou-o.

O MSE euclidiano inverte o ângulo de +41.6° para −16.1° durante o treino (rotação de −57.7°). O ECO-NN + regularização angular reverteu essa inversão (rotação de +58.4°), devolvendo o campo ao ângulo pré-treino.

Este é o **Limiar de Neutralização**: o ponto em que a dinâmica φ integrada neutraliza o achatamento euclidiano, mas não vai além. Para cruzar θ_R = 63.43°, a força angular precisa exceder a força de restauração.

#### II.5 — Por que os Betas São Pequenos

A rede aprendeu β ≈ 0.18–0.23, usando apenas ~30% do espaço SEAL (0.618). Isso indica que o MSE de reconstrução (L_rec = 0.27) domina o balanço. A ECO-dinâmica está ativa e contribui, mas o gradiente de reconstrução a comprime.

O caminho para cruzar θ_R: aumentar λ_angular ou reduzir o peso relativo do MSE — permitir que a força φ opere com mais liberdade, aceitando maior L_rec em troca de alinhamento angular.

#### II.6 — O que Permanece Invariante

Em todos os experimentos, com todas as lentes, com e sem ECO, com e sem regularização:

> **θ_local = 90°** — os micro-padrões da superfície são horizontais.

Isso não é um resultado negativo. É uma propriedade da arquitetura: a compressão [55→3] produz lâminas horizontais por construção geométrica. Grade R existe na trajetória global (crista), não na textura local. São escalas de observação distintas e independentes.
